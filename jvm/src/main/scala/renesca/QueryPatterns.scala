...
class QueryPatterns(resolvedItems: mutable.Map[Item,Origin]) {
  ...
  def nodePattern(node: Node, forDeletion: Boolean = false): NodePattern = {
    ...
    resolvedItems.getOrElse(node, node.origin) match {
      case Id(id)                =>
        NodePattern("match", s"($variable)", s"where id($variable) = {${ variable }_nodeId}", ParameterMap(s"${ variable }_nodeId" -> id.asJson), variable)
      case Create()              =>
        NodePattern("create", s"($variable $labels {${ variable }_properties})", "", ParameterMap(s"${ variable }_properties" -> node.properties.toMap.asJson), variable)
      case Merge(merge, onMatch) =>
        ...
        NodePattern("merge", s"($variable $labels $mergeLiteralMap)", s"on create set $variable += {${ variable }_onCreateProperties} on match set $variable += {${ variable }_onMatchProperties}",
          parameterMap ++ ParameterMap(s"${ variable }_onCreateProperties" -> remainingProperties.toMap.asJson, s"${ variable }_onMatchProperties" -> onMatchProperties.toMap.asJson),
          variable)
      case Match(matches)        =>
        ...
        val (propertySetter, propertyMap) = if (forDeletion || remainingProperties.isEmpty)
          ...
          (s"set $variable += {${ variable }_properties}", ParameterMap(s"${ variable }_properties" -> remainingProperties.toMap.asJson))
  ...
  def relationPattern(relation: Relation, forDeletion: Boolean = false): RelationPattern = {
    ...
    resolvedItems.getOrElse(relation, relation.origin) match {
      case Id(id)                =>
        RelationPattern("match", s"[$variable]", s"where id($variable) = {${ variable }_relationId}", ParameterMap(s"${ variable }_relationId" -> id.asJson), variable)
      case Create()              =>
        RelationPattern("create", s"[$variable :`${ relation.relationType }` {${ variable }_properties}]", "", ParameterMap(s"${ variable }_properties" -> relation.properties.toMap.asJson), variable)
      case Merge(merge, onMatch) =>
      ...
      case Match(matches)        =>
        ...
        RelationPattern("match", s"[$variable :`${ relation.relationType }` $matchLiteralMap]",
          propertySetter,
          parameterMap ++ propertyMap,
          variable)
    }
