...
import org.neo4j.driver.v1.util.{Pair => Neo4jPair}
...
object Neo4jTranslation {
  ...
  def recordsToGraph(records: Seq[Record]): (Graph, Map[Item, String]) = {
    ...
    val nodes = records.flatMap(_.fields collect {
      case p: Neo4jPair[_, _] if p.key.isInstanceOf[String] && p.value.isInstanceOf[neo4j.Node] =>
        val varName = p.key.asInstanceOf[String]
        val n = p.value.asInstanceOf[neo4j.Node]
    ...
    val edges = records.flatMap(_.fields collect {
      case p: Neo4jPair[_, _] if p.key.isInstanceOf[String] && p.value.isInstanceOf[neo4j.Relationship] =>
        val varName = p.key.asInstanceOf[String]
        val r = p.value.asInstanceOf[neo4j.Relationship]
